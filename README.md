...
- **Multi-currency** — EUR and CHF supported, with an adaptive display that shows each day's dominant currency and always shows the equivalent in the other currency as a sub-note
- **GPS tagging** — location is captured automatically when logging and can be opened in Apple Maps or Google Maps
- **Full dashboard** — scrollable WebView with three sections: By Category, By Day, All Entries
...
| File | Role |
|------|------|
| `ExpenseTracker.js` | Main script: dashboard, log, edit, settings, export |
...
- **Exchange rate**: set the current CHF → EUR conversion rate.
- **Default currency**: EUR or CHF — used automatically when logging.
- **Adaptive avg currency**: which currency to use for AVG/DAY and OVERALL totals.

### Exporting data

- Tap **📤** in the dashboard action bar.
- A CSV file is saved to iCloud Drive / Scriptable / `VacationExpenses_YYYYMMDD_HHMMSS.csv`.
- Each export creates a new file — no data is ever overwritten.
- Columns: Date, Time, Category, Currency, Amount, Amount (EUR), Latitude, Longitude, Description.
...
The tracker supports EUR and CHF natively. The display mode is cycled with the button in the top-right corner of the dashboard.

### ADAPTIVE mode (default)

Each value is shown in the most natural currency for its context, with the equivalent in the other currency shown as a small sub-note:

| Block | Currency shown | Sub-note |
|-------|---------------|----------|
| TODAY | Today's dominant currency* | Equivalent in the other currency |
| AVG/DAY | adaptiveAvgCur (from settings) | Equivalent in the other currency |
| OVERALL | adaptiveAvgCur (from settings) | Equivalent in the other currency |
| BY DAY rows | Each day's dominant currency | — |
| ALL ENTRIES | Native (original) currency of each entry | — |

\* *Dominant currency*: whichever currency has the higher EUR-equivalent total for that day. If today has no entries, the most recent past day's dominant currency is used. Falls back to EUR if no history exists.

Amounts are summed natively in the dominant currency — only the minority currency is converted, minimising rounding errors.

### EUR / CHF modes

All values are forced into the selected currency. No sub-notes.
...
### BY CATEGORY
A table showing today's spend, average per day (past days only), and overall total for each category. All values update instantly when you cycle the display mode — no page reload.

### BY DAY
A horizontally scrollable table with one row per day and one column per category, plus a daily total. The Date column is sticky (stays visible while scrolling). Today's row is shown in green, past days in yellow. Dates are formatted as `Thu 02 Apr`.

### ALL ENTRIES
A full chronological list of every entry, grouped by day. Each day header is tappable to collapse or expand that day's entries. The day header shows the weekday, date, and daily total. Each entry row shows the time, category dot, description, and native amount.
...
Shows:
- Today's total in the dominant currency, with the other-currency equivalent inline
...
`expenses.json` is a plain JSON array. Each entry:
...
`lat` and `lon` are omitted when location is unavailable.
